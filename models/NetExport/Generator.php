 ...
 * @subpackage Generators
 */
abstract class NetExport_Generator
{
    const CLASS_PREFIX = 'NetExport_Generator_';
     ...
     *
     * @param NetExport_File $reportFile The report file to be generated
     */
    ...
    public function generate()
    {
        ...
        try {
            $this->_reportFile->status = NetExport_File::STATUS_IN_PROGRESS;
            $this->_reportFile->forceSave();
    
            $this->_report = $this->_reportFile->getReport();
            $this->_params = netexport_convert_search_filters(unserialize($this->_report->query));
            ...
            $this->_reportFile->status = NetExport_File::STATUS_COMPLETED;
            $this->_reportFile->filename = $filename;
        } catch (Exception $e) {
            $this->_reportFile->status = NetExport_File::STATUS_ERROR;
